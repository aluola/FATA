"""CLI entry point for deterministic unique-image ScienceQA-MC construction."""
...
from .common import LlavaConstructionRunner, SEED, configure_reproducibility
from .sources import load_scienceqa
...
def main(argv: list[str] | None = None) -> int:
    ...
    output = resolve_output_root(
        args.output, resume=args.resume, dataset_name="ScienceQA_MC"
    )
    exclusions = (
        load_stored_build_exclusions(output, dataset_name="ScienceQA_MC")
        if args.resume
    ...
    bundle = load_scienceqa(args.cache)
        ...
        json.dumps(
            {
                ...
                "split_composition": result.stats["split_composition"],
            },
            ensure_ascii=False,
        )
    )
    if not result.target_reached:
        raise RuntimeError("ScienceQA_MC target not reached; resume the saved checkpoint")
    return 0
