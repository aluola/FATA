"""CLI entry point for deterministic unique-image TextVQA construction."""
...
from .sources import load_textvqa
...
def main(argv: list[str] | None = None) -> int:
    ...
    output = resolve_output_root(
        args.output, resume=args.resume, dataset_name="TextVQA_Open"
    )
    exclusions = (
        load_stored_build_exclusions(output, dataset_name="TextVQA_Open")
        if args.resume
    ...
    bundle = load_textvqa(args.cache)
    ...
    if not result.target_reached:
        raise RuntimeError("TextVQA_Open target not reached; resume the saved checkpoint")
    return 0
